...
class TrackPolymorphismTest {
    ...
    void fileTrackHasExplicitPropertiesAndWithers() {
        FileTrack track = new FileTrack("f1", Map.of("TITLE", List.of("Song")), 120L,
                TrackState.healthy(), "music/song.mp3", "mp3");
        ...
        FileTrack updated = track.withState(new TrackState(true, new TrackError("READ_ERROR", "Unreadable", Instant.now(), "SCAN")));
    ...
    void streamTrackEvaluatesFreshnessAndWithers() {
        ...
        StreamTrack fresh = new StreamTrack("s1", Map.of(), 200L, TrackState.healthy(),
                "https://source/1", "https://stream/1", now.plusSeconds(60));
        StreamTrack expired = new StreamTrack("s2", Map.of(), 200L, TrackState.healthy(),
                "https://source/2", "https://stream/2", now.minusSeconds(1));
        StreamTrack noExpiry = new StreamTrack("s3", Map.of(), 200L, TrackState.healthy(),
                "https://source/3", "https://stream/3", null);
